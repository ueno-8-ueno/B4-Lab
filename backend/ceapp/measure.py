...
OUTPUT_CSV_FILE = "../../result.csv" # ceappディレクトリからの相対パス
# --- 設定項目終わり ---
...
def write_log_csv(timestamp, source_container, target_container, rtt_avg_ms, packet_loss_percent,
    ...
    def val_or_empty(val): # Noneを空文字列に変換するヘルパー
...
# main_loop_process の修正 (write_log_csv を使用)
def main_loop_process(stop_event_param):
    ...
    while not stop_event_param.is_set():
        ...
        ping_result = run_clab_command(CLIENT_CONTAINER_NAME, ping_cmd, timeout_override=3) # Ping timeout
        ...
        if iperf_server_started_flag:
            ...
            if raw_udp_throughput is not None:
                ...
                jitter = raw_jitter # keep original precision if not None
        ...
        # 全てのメトリクスを1回のwrite_log_csv呼び出しで記録
...
def start_measures_route():
    ...
    # オプション: 測定開始時に古いCSVをバックアップ
    # output_file_path = os.path.join(os.path.dirname(__file__), OUTPUT_CSV_FILE)
    # if os.path.exists(output_file_path):
    #     backup_name = output_file_path + "." + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + ".bak"
    #     try:
    #         os.rename(output_file_path, backup_name)
    #         print(f"Backed up existing CSV to {backup_name}")
    #     except OSError as e:
    #         print(f"Could not backup CSV: {e}")
            
    loop_thread = threading.Thread(target=main_loop_process, args=(stop_event,), daemon=True)
    loop_thread.start()
    time.sleep(0.5) # スレッド開始のための短い待機
    
    if is_loop_running_check():
        return jsonify({'status': 'success', 'message': 'Measurement started.'})
    else:
        loop_thread = None # 念のため
        iperf_server_started_flag = False # 念のため
...
def stop_measures_route():
    ...
    if iperf_server_started_flag:
        ...
        if kill_output is not None and "no process found" not in str(kill_output).lower() and \
           ("terminated" in str(kill_output).lower() or not str(kill_output).strip()): # 成功 or プロセスなし
             ...
             if status_type == "success": status_type = "warning" # ステータスを格下げ
        iperf_server_started_flag = False # フラグをリセット
    
    if status_type == "success" and not (loop_thread and loop_thread.is_alive()): # 全て正常終了
...
def parse_csv_value_for_json(value_str):
    """JSON送信用にCSVの値をパース。数値にできなければNone"""
    if value_str is None or value_str == '':
        return None
    try:
        # 小数点が含まれる可能性のあるものはfloatへ
        if '.' in value_str:
            return float(value_str)
        return int(value_str) # それ以外はまずintへ
    except ValueError:
        # intにもfloatにもできない場合は元の文字列を返すか、Noneにするか選択
        # ここではNoneにしてフロントで扱いやすくする
        return None # または value_str のまま返す場合は `return value_str`
...
def get_csv_data_api():
    ...
    try:
        with open(csv_file_path, mode='r', encoding='utf-8-sig') as csvfile:
            ...
            # CSVのヘッダー名を小文字にして、元のヘッダー名とのマッピングを保持
            # これにより、CSVヘッダーの大文字・小文字の違いに対応
            csv_header_map = {f.lower().strip(): f for f in reader.fieldnames}

            # フロントエンドが期待するメトリクスのキー (スネークケース)
            ...
            for row_dict in reader:
                ...
                if not ts_original_key or row_dict.get(ts_original_key) is None:
                    app.logger.debug(f"Skipping row due to missing timestamp: {row_dict}")
                    continue # タイムスタンプがない/空の行はスキップ
                processed_row['timestamp'] = row_dict[ts_original_key]
                
                # source_container と target_container
                sc_original_key = csv_header_map.get('source_container')
                if sc_original_key:
                    processed_row['source_container'] = row_dict.get(sc_original_key)
                
                tc_original_key = csv_header_map.get('target_container')
                if tc_original_key:
                    processed_row['target_container'] = row_dict.get(tc_original_key)

                # 各メトリクス値を処理
                for key in expected_metric_keys:
                    original_metric_key = csv_header_map.get(key) # CSV内の実際のヘッダー名
                    if original_metric_key:
                        processed_row[key] = parse_csv_value_for_json(row_dict.get(original_metric_key))
                    else:
                        # CSVにカラムが存在しない場合はNoneを設定
                        processed_row[key] = None
                
                data_rows.append(processed_row)
        
        # 通常、CSVは追記されていくので、時系列でソートする必要はないかもしれないが、
        # もしソートが必要な場合はここで実施
        # data_rows.sort(key=lambda x: x['timestamp'])
